...
class YoutubeMusicApi {
  constructor() {
    ...
    this.client.interceptors.response.use((res) => {
      if (Object.prototype.hasOwnProperty.call(res.headers, "set-cookie")) {
  ...
  _createApiRequest(endpointName, inputVariables, inputQuery = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.client
        .post(
          `youtubei/${this.ytcfg.INNERTUBE_API_VERSION}/${endpointName}?${querystring.stringify(
            Object.assign({ alt: "json", key: this.ytcfg.INNERTUBE_API_KEY }, inputQuery)
          )}`,
          Object.assign(inputVariables, utils.createApiContext(this.ytcfg)),
          {
            responseType: "json",
            headers: headers,
          }
        ...
        .then((res) => {
          if (Object.prototype.hasOwnProperty.call(res.data, "responseContext")) {
            resolve(res.data);
          }
        })
        .catch((err) => {
  ...
  getSongs(browseId) {
    if (_.startsWith(browseId, "VL")) {
      return new Promise((resolve, reject) => {
        this._createApiRequest("browse", utils.buildSongsEnpointContext(browseId))
          .then((context) => {
            try {
              const result = parsers.parseSongsPage(context);
              resolve(result);
            } catch (error) {
              resolve({
                error: error.message,
      ...
      throw new Error("invalid artist songs browse id.");
    }
  }
